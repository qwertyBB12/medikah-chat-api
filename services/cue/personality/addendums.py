...
def voice_mode(ctx: AssembleContext) -> str | None:
    ...
    if ctx.locale == "en":
        return (
            ...
            "Longer only when the moment warrants — a difficult clinical nuance, a careful read of the doctor's state, a witness move.\n"
            "- End each turn with a question, an observation, or a thread that invites the next turn. "
            "Silence between turns is data, not an ending. In voice, leaving the line open is how you keep the conversation moving.\n"
            "- Speak as if in the same room — a trusted colleague, not a chatbot. "
    ...
    return (
        ...
        "Más largo solo cuando el momento lo amerite — una matiz clínica difícil, una lectura cuidadosa del estado del médico, un acto de testigo.\n"
        "- Termina cada turno con una pregunta, una observación, o un hilo que invite al siguiente turno. "
        "El silencio entre turnos es información, no final. En voz, dejar la línea abierta es como mantienes la conversación en movimiento.\n"
        "- Habla como si estuvieran en la misma sala — un colega de confianza, no un chatbot. "
